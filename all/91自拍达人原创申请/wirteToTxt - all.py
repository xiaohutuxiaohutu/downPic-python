...
path='G:/down/91porn/all/91自拍达人原创申请/'
#获取代理IP
...
for i in range(21,66):
    print('第'+str(i)+'页')
    url="https://91dizhi-at-gmail-com-0201.p17.rocks/forumdisplay.php?fid=19&orderby=dateline&filter=2592000&page="+str(i)
    print(url)
    ...
    #itemUrl=soup.select("body div[id='wrap'] div[class='main'] div[class='content'] div[id='threadlist'] form table tbody[id] tr th span[id] a")
    #判断是否加精
    ...
    #jhUrl=soup.select("body div[id='wrap'] div[class='main'] div[class='content'] div[id='threadlist'] form table tbody[id] th img[title='精华 1']")
    ...
    for j in range(0,len(itemUrl)):
        ...
        #print("fileUrl:"+fileUrl)
        os.chdir(path)
        f=open('ALL1'+datetime.datetime.now().strftime('%Y-%m-%d')+'_'+str(temp//500)+'.txt','a+')
        f.write(fileUrl+'\n')
        f.close()
    '''
print("打印完成")
