...
file=open("G:/down/91porn/all/91自拍达人原创申请/91porn_2018-12-30_0.txt")
...
for num,value in enumerate(file,1):
    ...
    while bl:
        try:
            ...
            npos=title.index('-')
            newTitle=title[0:npos]
            
            print(str(newTitle.strip()))
            #
            imgUrls=itemSoup.select("body div[id='wrap'] div[id='postlist'] div[id] table tbody tr td[class='postcontent'] div[class='defaultpost']  div[class] div[class] table tbody tr td img[file]")
            imgUrls1=itemSoup.select("body div[id='wrap'] div[id='postlist'] div[id] table tbody tr td[class='postcontent'] div[class='defaultpost'] div[class] div[class] div[class='postattachlist'] dl dd p img[file]")
            print('图片数量1：'+str(len(imgUrls)))
            print('图片数量1：'+str(len(imgUrls1)))
            path='D:/down/all/'+datetime.datetime.now().strftime('%Y-%m-%d')+'/'+str(newTitle.strip())+'/'
            for i in range(0,len(imgUrls)):
                fileUrl=imgUrls[i].get('file')
                fileUrl='http://f.91p02.club/'+fileUrl
                image_name=fileUrl.split("/")[-1]
                print('开始下载-第'+str(i+1)+'个:'+fileUrl)
                imageUrl=requests.get(fileUrl,headers=header,proxies=proxyip)
